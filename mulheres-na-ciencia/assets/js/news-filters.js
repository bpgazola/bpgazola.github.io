filterSelectionNews("all")

function filterSelectionNews(f) {
    var x, i;
    x = document.getElementsByClassName("element-item-news");
    if (f == "all") f = "";
    for (i = 0; i < x.length; i++) {
        elementRemoveClass(x[i], "show");
        if (x[i].className.indexOf(f) > -1) elementAddClass(x[i], "show");
    }
}
